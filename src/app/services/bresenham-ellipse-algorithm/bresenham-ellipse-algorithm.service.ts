...
export class BresenhamEllipseAlgorithmService {
  public getRenderPoints(center: Vector, radiuses: Vector): Vector[] {
    ...
    d2 =
      pow2(radiuses.y) * ((x + 0.5) * (x + 0.5)) +
      pow2(radiuses.x) * ((y - 1) * (y - 1)) -
      pow2(radiuses.x) * pow2(radiuses.y);
    ...
    return pixelsBatch.map(pixel => ({ x: Math.ceil(pixel.x), y: Math.ceil(pixel.y) }));
  }
}
